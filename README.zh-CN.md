...
- **牌组管理** - 支持层级牌组浏览、查看 new/learn/review 数量、创建、重命名、移动与导出牌组或子牌组，支持对牌组项进行左滑操作，并可将牌组拖动到其他牌组下，同时可管理牌组预设和调度选项。
- **专注的复习体验** - 支持 Again/Hard/Good/Easy、间隔预览、音频回放、typed answer、旗标、埋藏/暂停、撤销、修改到期日、可配置的轻点区域（上中下 3 格或 9 宫格），以及复习中的笔记/卡片编辑工具。
- **可配置的 AI 工作流** - 支持在复习、阅读器和笔记编辑中的选中文本菜单里加入 AI 动作，连接 OpenAI 兼容的 chat completions 接口，按预设管理模型 / 系统提示词 / 术语约束，配置快捷动作、收藏回答，并将 AI 回答按字段映射和标签生成笔记草稿。
- **Anki JS API 兼容层** - 在复习卡片页兼容一组常用的 AnkiDroid JavaScript API 能力，支持通过模板脚本读取 reviewer 状态、控制显示答案与评分、调用 TTS、执行搜索、修改旗标/标记/标签等交互。
- **强大的浏览与批量操作** - 支持全库搜索、按牌组和标签过滤、排序、懒加载结果、多选，并执行批量标签编辑、删除、移动牌组、修改笔记类型、导出、暂停和重置为新卡等操作。
- **富文本笔记编辑** - 支持基于后端真实字段名创建和编辑笔记，提供富文本字段编辑、标签管理、媒体插入、录音、源码模式、提交前卡片预览，以及带裁剪、压缩、预设缩放和自定义最长边限制（不会放大小图）的图片优化流程。
- **图像遮挡支持** - 支持原生创建和编辑 Image Occlusion 笔记，内置矩形、椭圆、多边形与文本遮罩工具。
- **模板与笔记类型工具** - 可在应用内直接查看和编辑卡片模板、CSS 与字段定义，并提供模板管理和字段管理界面，支持新增笔记类型、新增卡片模板，以及在应用内完成字段定义管理。
...
- **[Anki JS API 说明](docs/Anki-JS-API.md)**
